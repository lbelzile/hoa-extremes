...
gomp_cens <- function(par, dat, rightcens, slow, expo = FALSE){
  ...
  dgomp <- function(x, scale, shape, log = FALSE){
    ll <- log(shape) + shape/scale*(1-exp(scale*x))+ scale*x
    if(log){
      return(ll)
    } else{
      return(exp(ll))
...
#' @return a vector of length three containing twice the negative log-likelihood value, the endpoint value and the maximum of the nuisance lambda (i.e., the scale parameter).
...
#' @inheritParams gpd_cens
#' @return a p-value
rfun_xi <- function(psi, thetahat, dat, rightcens, slow){
  if(abs(psi) > 1e-6){
